...
class VehicleRepository extends BaseRepository
{
    ...
    {
        try {
            return $this->model->with(['driver','device','notifs'])
                ->find($id);
        } catch (Exception $e){
            throw new ExceptionHandler($e->getMessage(), $e->getCode());
        }
    ...
    public function status($id, $status)
    {
        try {
            ...
            $vehicle = $this->model->with(['driver','device','notifs'])
                ->findOrFail($id);
    ...
    public function deleteAll($ids)
    {
        try {
            ...
            $vehicles =  $this->model->whereIn('id', $ids)->get();
            foreach ($vehicles as $vehicle) {
                $vehicle->delete(); // Triggers deleting and deleted events
            }
            return true;
    ...
    public function getRelicateProductName($name)
    {
        ...
        } while ($this->model->where('title', $name)->exists());
    ...
    public function replicate($ids)
    {
        ...
        try {
            ...
            foreach($ids as $id) {
                ...
                $newVehicle->title = $this->getRelicateProductName($newVehicle->title);
                $newVehicle->created_at = Carbon::now();
                $newVehicle->driver_id = $vehicle->driver_id;
                $newVehicle->save();

//                // Handle the Device relationship (belongsTo)
//                if ($vehicle->device) {
//                    // Replicate the associated device
//                    $newDevice = $vehicle->device->replicate();
//                    $newDevice->save();
//
//                    // Assign the new device to the replicated vehicle
//                    $newVehicle->device()->associate($newDevice);
//                    $newVehicle->save();
//                }

                // Handle the Notifs relationship (belongsToMany)
                if ($vehicle->notifs()->exists()) {
                    // Retrieve notifs with pivot data
                    $notifsWithPivot = $vehicle->notifs()->withPivot(['alert', 'method'])->get();

                    // Attach the notifications to the new vehicle with pivot data
                    foreach ($notifsWithPivot as $notif) {
                        $newVehicle->notifs()->attach($notif->id, [
                            'alert' => $notif->pivot->alert,
                            'method' => $notif->pivot->method,
                        ]);
                    }
                }

                $products[] = $newVehicle->fresh();
            }
